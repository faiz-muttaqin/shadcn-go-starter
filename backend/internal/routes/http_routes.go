...
func Routes() {
	...
	api := r.Group("", middleware.AuthClerk())
	{
		...
		api.GET("/users", handler.GET_DEFAULT_TableDataHandler(database.DB, &model.User{}, []string{"Role"}))
		api.POST("/users", handler.POST_DEFAULT_TableDataHandler(database.DB, &model.User{}, []string{"Role"}))
		api.PATCH("/users", handler.PATCH_DEFAULT_TableDataHandler(database.DB, &model.User{}, []string{"Role"}))
		api.PUT("/users", handler.PUT_DEFAULT_TableDataHandler(database.DB, &model.User{}, []string{"Role"}))
