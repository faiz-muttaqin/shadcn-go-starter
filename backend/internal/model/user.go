...
type User struct {
	...
	Avatar             types.Avatar   `gorm:"column:avatar;size:255" json:"avatar" ui:"visible;editable"`
	Email              types.Email    `gorm:"column:email;size:100;unique" json:"email" ui:"creatable;visible;editable;filterable;sortable"`
	Username           string         `gorm:"column:username;size:50" json:"username" ui:"creatable;visible;editable;filterable;sortable"`
	FirstName          string         `gorm:"column:first_name;size:50" json:"first_name" ui:"creatable;visible;editable;filterable;sortable"`
	LastName           string         `gorm:"column:last_name;size:50" json:"last_name" ui:"creatable;visible;editable;filterable;sortable"`
	PhoneNumber        types.Phone    `gorm:"column:phone_number;size:20" json:"phone_number" ui:"creatable;visible;editable;filterable;sortable"`
	Password           types.Password `gorm:"column:password;size:100" json:"-"`
	Status             types.Badge    `gorm:"column:status" json:"status" ui:"visible;editable;filterable;sortable;selection:/options?data=status"`
	Session            string         `gorm:"column:session;size:120" json:"session"`
	LastLogin          time.Time      `gorm:"column:last_login" json:"last_login" ui:"visible;sortable"`
	RoleID             uint           `gorm:"column:role_id;index" json:"role_id"`
	Role               UserRole       `gorm:"foreignKey:RoleID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:SET NULL" json:"role" ui:"visible;editable;filterable;sortable;selection:/options?data=role"`
