...
import { type Theme, type ThemeStyles } from "@/types/theme";
...
const Editor: React.FC<EditorProps> = ({ themePromise }) => {
  ...
  const [initialTheme, setInitialTheme] = useState<Theme | null | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    themePromise
      .then((res) => {
        if (!mounted) return;
        setInitialTheme(res ?? null);
      })
      .catch((err) => {
        console.error("Failed to resolve themePromise:", err);
        if (mounted) setInitialTheme(null);
      });
