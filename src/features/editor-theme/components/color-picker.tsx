...
import { useColorControlFocus } from "@/stores/color-control-focus-store";
...
const ColorPicker = ({ color, onChange, label, name }: ColorPickerProps) => {
  ...
  useEffect(() => {
    ...
    if (isHighlighted) {
      // Avoid synchronous setState inside the effect which may cause cascading renders.
      // Use startTransition to defer non-urgent UI updates and capture the current
      // expanded state so we can decide the scroll delay consistently.
      const wasExpanded = !!sectionCtx?.isExpanded;
      ...
      // Defer non-urgent UI updates to avoid synchronous setState inside the effect
      React.startTransition(() => setShouldAnimate(false));
    }
