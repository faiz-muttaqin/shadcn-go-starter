...
const ThemePresetCycleControls: React.FC<ThemePresetCycleControlsProps> = ({
  ...
  const currentIndex = filteredPresets.indexOf(currentPresetName || "default");
  ...
  const cycleTheme = useCallback(
    (direction: "prev" | "next") => {
      const newIndex =
        direction === "next"
          ? (safeCurrentIndex + 1) % filteredPresets.length
          : (safeCurrentIndex - 1 + filteredPresets.length) % filteredPresets.length;
      applyThemePreset(filteredPresets[newIndex]);
    },
    [safeCurrentIndex, filteredPresets, applyThemePreset]
  );
  return (
    <>
...
const ThemePresetSelect: React.FC<ThemePresetSelectProps> = ({
  ...
  }, [loadSavedPresets, unloadSavedPresets, user]);
