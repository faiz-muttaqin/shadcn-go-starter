...
export function FontPicker({
  ...
  // Destructure the pieces of the query we rely on so the callback deps
  // can be specific and stable (avoids the React Compiler memoization warning).
  const hasNextPage = fontQuery.hasNextPage;
  const isFetchingNextPage = fontQuery.isFetchingNextPage;
  const fetchNextPage = fontQuery.fetchNextPage;
  ...
  const loadMoreRefCallback = useCallback(
    (node: HTMLDivElement | null) => {
      ...
      return () => {
        // Disconnect the observer entirely when the node unmounts to ensure
        // we don't leak observers or hold references to detached nodes.
        observer.disconnect();
      };
    },
