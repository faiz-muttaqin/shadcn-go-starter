...
const MarqueeRow = ({
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && containerRef.current) {
      ...
      const timer = window.setTimeout(() => {
        setDuplicateCount((prev) => (prev !== loopCount ? loopCount : prev));
        setContainerWidth((prev) => (prev !== totalWidth ? totalWidth : prev));
      }, 0);

      x.set(reverse ? -totalWidth / 2 : 0);

      return () => clearTimeout(timer);
    }
  }, [items.length, reverse, x]);
  ...
  x.set(newX);
    ...
    // Initialize lastTime here (impure call inside effect is allowed)
    lastTime.current = performance.now();
    animationFrame.current = requestAnimationFrame(animate);
    return () => cancelAnimationFrame(animationFrame.current);
  }, [containerWidth, reverse, shouldReduceMotion, speed, x]);
